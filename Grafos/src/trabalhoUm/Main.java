...
public class Main {
	public static void listarLista(ArrayList <Aresta> ar, ArrayList<Integer> vl){
		String s="";
	...
	public static void listarMtzAdj(int mat[][], int v){
		String s="";
		for(int i=0;i<v;i++){
			for(int j=0;j<v;j++){
				if(j!=(v-1)){
	...
	public static void listarMtzArs(int mat[][], int a, int v){
		String s="";
		for(int i=0;i<v;i++){
			for(int j=0;j<a;j++){
				if(j!=(a-1)){
	...
	public static void main(String[] args) {
		...
		if((valorado&&!direcionado)||(valorado&&direcionado)){
			...
			listarLista(arestas, listaValores);
			listarMtzAdj(matAdj, v);
			listarMtzArs(matAr, arestas.size(), v);
			JOptionPane.showMessageDialog(null, "E:"+E+"\n"+"V:"+V);
		}
